...
static void XMLCALL __AddCutElementEnd(void *data, const char *el)
{
  ...
  if(strcasecmp(el,"evergreen")==0) {
    cut->cut_evergreen=__AddCutReadBool(xml_data->strbuf);
  }
  ...
  if(strcasecmp(el,"sun")==0) {
    cut->cut_sun=__AddCutReadBool(xml_data->strbuf);
  }
  if(strcasecmp(el,"mon")==0) {
    cut->cut_mon=__AddCutReadBool(xml_data->strbuf);
  }
if(strcasecmp(el,"tue")==0) {
    cut->cut_tue=__AddCutReadBool(xml_data->strbuf);
  }
  if(strcasecmp(el,"wed")==0) {
    cut->cut_wed=__AddCutReadBool(xml_data->strbuf);
  }
  if(strcasecmp(el,"thu")==0) {
    cut->cut_thu=__AddCutReadBool(xml_data->strbuf);
  }
  if(strcasecmp(el,"fri")==0) {
    cut->cut_fri=__AddCutReadBool(xml_data->strbuf);
  }
  if(strcasecmp(el,"sat")==0) {
    cut->cut_sat=__AddCutReadBool(xml_data->strbuf);
  }
