...
unsigned __AddCartReadBool(const char *val)
{
...
static void XMLCALL __AddCartElementEnd(void *data, const char *el)
{
  ...
  if(strcasecmp(el,"groupName")==0) {
    strncpy(cart->cart_grp_name,xml_data->strbuf,11);
  }
  if(strcasecmp(el,"title")==0) {
    strncpy(cart->cart_title,xml_data->strbuf,256);
  }
  if(strcasecmp(el,"artist")==0) {
    strncpy(cart->cart_artist,xml_data->strbuf,256);
  }
  if(strcasecmp(el,"album")==0) {
    strncpy(cart->cart_album,xml_data->strbuf,256);
  }
  if(strcasecmp(el,"year")==0) {
    sscanf(xml_data->strbuf,"%d",&cart->cart_year);
  }
  if(strcasecmp(el,"label")==0) {
    strncpy(cart->cart_label,xml_data->strbuf,65);
  }
  if(strcasecmp(el,"client")==0) {
    strncpy(cart->cart_client,xml_data->strbuf,65);
  }
  if(strcasecmp(el,"agency")==0) {
    strncpy(cart->cart_agency,xml_data->strbuf,65);
  }
  if(strcasecmp(el,"publisher")==0) {
    strncpy(cart->cart_publisher,xml_data->strbuf,65);
  }
  if(strcasecmp(el,"composer")==0) {
    strncpy(cart->cart_composer,xml_data->strbuf,65);
  }
  if(strcasecmp(el,"userDefined")==0) {
    strncpy(cart->cart_user_defined,xml_data->strbuf,256);
  }
  ...
  if(strcasecmp(el,"owner")==0) {
    strncpy(cart->cart_owner,xml_data->strbuf,66);
  }
			...
			unsigned *numrecs)
{
  ...
  *numrecs = 0;
  ...
  snprintf(post,1500,"COMMAND=12&LOGIN_NAME=%s&PASSWORD=%s&GROUP_NAME=%s&TYPE=%s&CART_NUMBER=%u",
	   username,passwd,group,type,cartnumber);
