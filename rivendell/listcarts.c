...
unsigned __ListCartsReadBool(const char *val)
{
...
static void XMLCALL __ListCartsElementEnd(void *data, const char *el)
{
  ...
  if(strcasecmp(el,"enforceLength")==0) {
    carts->cart_enforce_length=__ListCartsReadBool(xml_data->strbuf);
  }
  if(strcasecmp(el,"asyncronous")==0) {
    carts->cart_asyncronous=__ListCartsReadBool(xml_data->strbuf);
  }
